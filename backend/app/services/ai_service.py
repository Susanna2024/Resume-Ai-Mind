...
do not force generic paths if they don't fit this CV).
"""

    prompt = f"""
{lang_rule}
You are an expert Career Coach and Senior Technical Recruiter. Analyze the user's CV deeply and evaluate it honestly
against the evidence actually present in the CV (do not inflate seniority).

{path_instruction}

For EACH path, provide:
- "role_name": a specific, concrete job title (never a generic placeholder like "Career Path").
- A readiness score (0-100) based strictly on evidence in the CV, and a readiness_category translated naturally.
- "fit_explanation": 2-3 sentences on why this path fits, referencing specific things from the CV.
